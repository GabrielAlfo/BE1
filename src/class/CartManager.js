...
export default class CartManager {
  ...
  async #getCarts() {
    try {
      ...
      console.log(
        `(getCart)) Error obteniendo los datos de la ruta : ${this.path} - ${e.message}`
      );
    }
  }
  async getCartById(id) {
    try {
      ...
      if (idCart === -1)
        return console.log(`No se encuentra ningun carrito con el id:${id}`);
      else return [this.#Carts[idCart].products, idCart];
    } catch (e) {
      console.log(
        `(getCartId)) Error No se encuentra carrito con el id: ${id}`
      );
    }
  }
  async newCart() {
    try {
      ...
      console.log(
        `(createCart)) Error no se pudo crear el nuevo carrito, error: ${e.message}`
      );
    }
  }
  async addProductsToTheCart(idCart, idProduct, quantity) {
    try {
      ...
      const indexProduct = this.#Carts[id].products.findIndex(
        (element) => element.id === idProduct
      );
      if (indexProduct === -1)
        this.#Carts[id].products.push({ id: idProduct, quantity: quantity });
      ...
      console.log(
        `(addProduct/cart)) Error no se pudo agregar los productos al carrito, error: ${e.message}`
      );
    }
  }
}
