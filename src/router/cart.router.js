...
router.post("/", async (req, res) => {
  ...
  if (!newCart)
    return res
      .status(400)
      .send({ message: "Error no se pudo crear un carrito nuevo" });
...
router.get("/:cip", async (req, res) => {
  ...
  if (!cart)
    return res
      .status(400)
      .send(`Error no se encontro ningun cart con el id: ${id}`);
...
router.post("/:cid/product/:pid", async (req, res) => {
  ...
  if (!addProduct)
    return res
      .status(400)
      .send({ message: "Error no se pudo agregar el producto al carrito" });
  else
    return res.send({
      status: "success",
      message: `Producto con el id:${idProduct} agregado correctamente al carrito: ${idCart}`,
    });
});
